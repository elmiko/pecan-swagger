...
def get_wsme_defs(name):
    ...
    def get_wsattr_and_wsproperty(obj):
        ...
        for key, value in inspect.getmembers(obj):
            if key[0] != '_' \
               and (value.__class__.__name__ == 'wsattr' \
    ...
    def inspect_wm_schema(schema_obj, isparams=False):
        ...
        for key, obj in inspect.getmembers(schema_obj):
            if key[0] != '_' \
               and (obj.__class__.__name__ == 'wsattr' \
                ...
                if ws_len == 1:
                    ...
                    if get_type_str(obj.datatype) != 'array':
                        # not array
                        schema_dict[to_swag_key(key)] = get_wm_item_prop(obj, isparams)
                    else:
                        # single array
                ...
                # sample function doesn't work without API service in OpenStack Magnum
    ...
    def get_wm_def(o):
        ...
        for w, d in inspect.getmembers(o):
            if w == 'arguments':
                wsme[to_swag_key(w)] = []
                for arg in d:
                    # set each 'Parameter Object', it can include 'Items Object' recursively
                    item_dict = get_wm_item_prop(arg, True)
                    # TODO: MUST be set one of 'body|query|path|header|formData'
                    item_dict['in'] = 'query'
                ...
                if d:
                    wsme['responses']['status'][to_swag_key(w)] = inspect_wm_schema(d, False)
